from avlNode import avlnode
from item import DirectoryItem
from hashTable import HashTable
from avlTree import avltree

record = DirectoryItem('TestUser1', 'Test1', 'address', 99901, 99991)
record2 = DirectoryItem('TestUser2', 'Test2', 'otraaddress', 99902, 8882)
record3 = DirectoryItem('TestUser3', 'Test3', 'otraaddress', 99903, 8883)
record4 = DirectoryItem('TestUser4', 'Test4', 'otraaddress', 99904, 8884)
record5 = DirectoryItem('TestUser5', 'Test5', 'otraaddress', 99905, 8885)
record6 = DirectoryItem('TestUser6', 'Test6', 'otraaddress', 99906, 8886)

tree = avltree()
hashTable = HashTable()
table = [0] * 10

hashTable.insert_in_avl_tree(table, 41, record)
hashTable.insert_in_avl_tree(table, 52, record2)
hashTable.insert_in_avl_tree(table, 53, record3)
hashTable.insert_in_avl_tree(table, 49, record4)
hashTable.insert_in_avl_tree(table, 81, record5)
hashTable.insert_in_avl_tree(table, 11, record5)
hashTable.insert_in_avl_tree(table, 31, record5)
hashTable.insert_in_avl_tree(table, 51, record5)
hashTable.insert_in_avl_tree(table, 111, record5)
hashTable.insert_in_avl_tree(table, 201, record5)
hashTable.insert_in_avl_tree(table, 91, record2)
hashTable.insert_in_avl_tree(table, 23, record6)
# print hashTable.search(table,201)

for item in table:
    if isinstance(item, avltree):
        print item.inorder_traverse()
    else:
        print item

# print table
hashTable.delete(table, 49)
hashTable.delete(table, 52)
hashTable.delete(table, 201)
record99 = DirectoryItem('TestUser99', 'Test99', 'otraaddress', 99909, 8889)
hashTable.modify_record(table, 23, record99)
hashTable.search(table, 201)
hashTable.search(table, 31)
# print table

for item in table:
    if isinstance(item, avltree):
        print item.inorder_traverse()
    else:
        print item
